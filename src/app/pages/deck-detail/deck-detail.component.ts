...
@Component({
  ...
  standalone: true,
  imports: [NgIf, NgFor, FormsModule, RouterLink, NavbarComponent, ResultCardComponent, LoaderComponent, FounderQuestionsComponent, TeamNotesComponent],
  template: `
    <div style="min-height:100vh;background:var(--bg);">
      ...
      <div *ngIf="!loading && deck" style="max-width:88vw;margin:0 auto;padding:36px 24px 64px;">
        ...
        <!-- Founder Questions sub-component -->
        <div style="margin-top:24px;">
          <app-result-card title="Founder Questions">
            <app-founder-questions
              [deckId]="deck.id"
              [questions]="questions"
              [emailedIndices]="deck.emailed_questions"
...
export class DeckDetailComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.deckService.getDeck(id).subscribe({
      next: d => { this.deck = d; this.questions = d.founder_questions.map(q => ({ ...q })); this.founderEmail = d.founder_email; this.loading = false; },
